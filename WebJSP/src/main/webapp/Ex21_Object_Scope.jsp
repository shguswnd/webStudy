...
<!-- 
	...
	2. response(응답 : 특정 페이지 이동, 정보출력, 쿠키 쓰기)
	...
	*session객체(활용도 ^^)
	...
	scope : a.jsp(session.setAtt.."member" 변수 생성)
			b.jsp(session.setAtt.."member" 변수 생성)
			
	session 변수는 (현재 Webapp 폴더 안에 있는 모든 페이지 사용가능)
	Life-cycle : 1. session변수는 session 객체와 동일
				 2. session 소멸 : 서버 리부팅, Tomcat 재시행, session.invaildate()
				 
	* application 객체
	[내부에 application 변수 생성 : 전역변수 >> 모든 session (접속한 모든 사용자) 공유
	
	scope : (현재 Webapp 폴더 안에 있는 모든 페이지에서 사용 가능) >> java >> static
	Life-cycle : 소명(서버 리부팅),  Tomcat 재실행
 ...
 <%
 	...
 	//setAttribute 두번쨰 인자는 오브젝트 객체 >> 단점 >> 제너릭 지원한되서 다운캐스팅 해야됨
 	//(String name, Object value)
 	
 	//session (예를 들어 다른 페이지라고 가정하고)
 	...
 	1. b.jsp에서 session 변수 >> empobj >> 사용(O) >> include와 상관없이
 	2. b.jsp 에서 request 변수 >> who >> 사용(O)
